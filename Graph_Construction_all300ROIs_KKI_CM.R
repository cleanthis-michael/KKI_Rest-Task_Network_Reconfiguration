...
# A1. specify the network membership of each ROI (using the 300ROI extended power atlas for this analysis)
...
roi_membership <- as.vector(bigbrainatlas[,2]); roi_membership
roi_comms <- as.numeric(as.factor(roi_membership)); roi_comms # creates a numeric vector of network membership for each roi

# A2. read in all correlation matrices created in the 'DataProcNetCon_MEMFYP_trhfix.R' script from ROI timecourses
corrmatfiles= list.files("./correlation_matrices_all300ROIs/", pattern=glob2rx("sub-*.csv"), full.names = TRUE)
...
unique(bigbrainatlas$Network.Assignment) 
# add a V in front of each ROI so that we can merge smoothly later
bigbrainatlas$ROI.Number2 <- paste("V", bigbrainatlas$ROI.Number, sep = "")

# make ROI vectors for each network (MEM commented out the networks that she is not interested in)
rois.frontoparietal <- bigbrainatlas$ROI.Number2[bigbrainatlas$Network.Assignment == "Fronto-Parietal"]
rois.cinguloopercular <- bigbrainatlas$ROI.Number2[bigbrainatlas$Network.Assignment == "Cingulo-Opercular"]
rois.dmn <- bigbrainatlas$ROI.Number2[bigbrainatlas$Network.Assignment == "Default Mode"]
rois.dorsalattn <- bigbrainatlas$ROI.Number2[bigbrainatlas$Network.Assignment == "Dorsal Attention"]
rois.somatdorsal <- bigbrainatlas$ROI.Number2[bigbrainatlas$Network.Assignment == "Somatomotor Dorsal"]
rois.somatventral <- bigbrainatlas$ROI.Number2[bigbrainatlas$Network.Assignment == "Somatomotor Ventral"]
rois.visual <- bigbrainatlas$ROI.Number2[bigbrainatlas$Network.Assignment == "Visual"]
#rois.salience <- bigbrainatlas$ROI.Number2[bigbrainatlas$Network.Assignment == "Salience"]
#rois.ventralattn <- bigbrainatlas$ROI.Number2[bigbrainatlas$Network.Assignment == "Ventral Attention"]



#rois.reward <- bigbrainatlas$ROI.Number2[bigbrainatlas$Network.Assignment == "Reward"]
#rois.unassigned <- bigbrainatlas$ROI.Number2[bigbrainatlas$Network.Assignment == "Unassigned"]
#rois.medtemp <- bigbrainatlas$ROI.Number2[bigbrainatlas$Network.Assignment == "Medial Temporal"]
#rois.auditory <- bigbrainatlas$ROI.Number2[bigbrainatlas$Network.Assignment == "Auditory"]
#rois.parietalmemory <- bigbrainatlas$ROI.Number2[bigbrainatlas$Network.Assignment == "Parietal Memory"]
...
for (i in 1:length(corrmatfiles)){
  ...
  subcorr_mat[which(subcorr_mat < 0.45, arr.ind = T)] = 0 #absolute thresholding
  ...
  dat.loop$mod = modularity(corr_nets, roi_comms) # looking at modularity when utilizing the big brain atlas network membership assignments (preassigned network membership from adult atlas Power et al. 2011 & extended with additional subcortical and cerebellar coverage in Seitzman et al. 2019)
  ...
  pc = part_coeff(corr_nets, roi_comms) # feed it the igraph object and the vector of roi network assignments
  ...
  ndi = NDI(subcorr_mat, roi_comms) #feed it the correlation matrix, not the graph; TRH wrote the function for this
  ...
  wmd = within_module_deg_z_score(corr_nets, roi_comms) # feed it the igraph object and the vector of roi network assignments
  ...
  # dorsal attention clustering coefficient
  dan_subgraph = induced_subgraph(graph = corr_nets, vids = rois.dorsalattn, impl = c("auto"))
  transit_dan <- transitivity(g = dan_subgraph, type = c("global"), vids = NULL, weights = NULL, isolates = c("NaN", "zero"))
  ...
  # somatomotor ventral clustering coefficient
  smv_subgraph = induced_subgraph(graph = corr_nets, vids = rois.somatventral, impl = c("auto"))
  transit_smv <- transitivity(g = smv_subgraph, type = c("global"), vids = NULL, weights = NULL, isolates = c("NaN", "zero"))
  ...
  temp_clustcoeff$clustcoeff_dan =  transit_dan
  temp_clustcoeff$clustcoeff_smd = transit_smd
  temp_clustcoeff$clustcoeff_smv = transit_smv
...
require(dplyr)
dat_pc <- dat_pc %>% select(sub, ses, task, V1:V300)
dat_pc[dat_pc == 0.0000000] <- NaN # set zeros to NA

dat_wmd <- dat_wmd %>% select(sub, ses, task, V1:V300)
dat_wmd[dat_wmd == 0.0000000] <- NaN # set zeros to NA

dat_le <- dat_le %>% select(sub, ses, task, V1:V300)

dat_ndi <- dat_ndi %>% select(sub, ses, task, V1:V300)
...
date=format(Sys.time(), "%m.%d.%Y")
save(dat, dat_pc, dat_wmd, dat_le, dat_ndi, dat_clustcoeff, file = sprintf("CMflux_graphoutputs_weighted_absthresh045_%s.RData", date)) #update name with (i) binarized or weighted, (ii) the threshold type and level
...
# D1. prep for reshape
dat_pc_byroi <- dat_pc
col_for_reshape_pc <- paste(colnames(dat_pc_byroi)[4:303], "_", roi_comms, sep = "")
colnames(dat_pc_byroi)[4:303] <- paste(colnames(dat_pc_byroi)[4:303], "_", roi_comms, sep = "") # adds network affiliation into the column name

# D2. reshape from wide to long keeping sub, ses, and task as columns
dat_pc_byroi
dat_pc_byroi_long <- reshape(dat_pc_byroi, 
                              direction = "long",
                              varying = list(names(dat_pc_byroi)[4:303]),
                              v.names = "pc",
...
pc_netavg <- ddply(dat_pc_byroi_long, .(sub, task), summarize, 
                    avg_pc_net1=mean(pc[grepl("_1", roi)], na.rm=TRUE), 
                    avg_pc_net2=mean(pc[grepl("_2", roi)], na.rm=TRUE), 
                    avg_pc_net3=mean(pc[grepl("_3", roi)], na.rm=TRUE),
                    avg_pc_net4=mean(pc[grepl("_4", roi)], na.rm=TRUE), 
                    avg_pc_net5=mean(pc[grepl("_5", roi)], na.rm=TRUE), 
                    avg_pc_net6=mean(pc[grepl("_6", roi)], na.rm=TRUE),
                    avg_pc_net7=mean(pc[grepl("_7", roi)], na.rm=TRUE), 
                    avg_pc_net8=mean(pc[grepl("_8", roi)], na.rm=TRUE), 
                    avg_pc_net9=mean(pc[grepl("_9", roi)], na.rm=TRUE),
                    avg_pc_net10=mean(pc[grepl("_10", roi)], na.rm=TRUE), 
                    avg_pc_net11=mean(pc[grepl("_11", roi)], na.rm=TRUE), 
                    avg_pc_net12=mean(pc[grepl("_12", roi)], na.rm=TRUE),
                    avg_pc_net13=mean(pc[grepl("_13", roi)], na.rm=TRUE), 
                    avg_pc_net14=mean(pc[grepl("_14", roi)], na.rm=TRUE))
...
# E1. prep for reshape
dat_le_byroi <- dat_le
col_for_reshape_le <- paste(colnames(dat_le_byroi)[4:303], "_", roi_comms, sep = "")
colnames(dat_le_byroi)[4:303] <- paste(colnames(dat_le_byroi)[4:303], "_", roi_comms, sep = "") # adds network affiliation into the column name

# E2. reshape from wide to long keeping sub, ses, and task as columns
dat_le_byroi
dat_le_byroi_long <- reshape(dat_le_byroi, 
                             direction = "long",
                             varying = list(names(dat_le_byroi)[4:303]),
                             v.names = "le",
                             idvar = c("sub", "ses", "task"),
                             timevar = "roi",
...
le_netavg <- ddply(dat_le_byroi_long, .(sub, task), summarize, 
                   avg_le_net1=mean(le[grepl("_1", roi)], na.rm=TRUE), 
                   avg_le_net2=mean(le[grepl("_2", roi)], na.rm=TRUE), 
                   avg_le_net3=mean(le[grepl("_3", roi)], na.rm=TRUE),
                   avg_le_net4=mean(le[grepl("_4", roi)], na.rm=TRUE), 
                   avg_le_net5=mean(le[grepl("_5", roi)], na.rm=TRUE), 
                   avg_le_net6=mean(le[grepl("_6", roi)], na.rm=TRUE),
                   avg_le_net7=mean(le[grepl("_7", roi)], na.rm=TRUE), 
                   avg_le_net8=mean(le[grepl("_8", roi)], na.rm=TRUE), 
                   avg_le_net9=mean(le[grepl("_9", roi)], na.rm=TRUE),
                   avg_le_net10=mean(le[grepl("_10", roi)], na.rm=TRUE), 
                   avg_le_net11=mean(le[grepl("_11", roi)], na.rm=TRUE), 
                   avg_le_net12=mean(le[grepl("_12", roi)], na.rm=TRUE),
                   avg_le_net13=mean(le[grepl("_13", roi)], na.rm=TRUE), 
                   avg_le_net14=mean(le[grepl("_14", roi)], na.rm=TRUE),
...
col_for_reshape_ndi <- paste(colnames(dat_ndi_byroi)[4:303], "_", roi_comms, sep = "")
colnames(dat_ndi_byroi)[4:303] <- paste(colnames(dat_ndi_byroi)[4:303], "_", roi_comms, sep = "") # adds network affiliation into the column name

# F2. reshape from wide to long keeping sub, ses, and task as columns
dat_ndi_byroi
dat_ndi_byroi_long <- reshape(dat_ndi_byroi, 
                             direction = "long",
                             varying = list(names(dat_ndi_byroi)[4:303]),
                             v.names = "ndi",
                             idvar = c("sub", "ses", "task"),
                             timevar = "roi",
...
ndi_netavg <- ddply(dat_ndi_byroi_long, .(sub, task), summarize, 
                   avg_ndi_net1=mean(ndi[grepl("_1", roi)], na.rm=TRUE), 
                   avg_ndi_net2=mean(ndi[grepl("_2", roi)], na.rm=TRUE), 
                   avg_ndi_net3=mean(ndi[grepl("_3", roi)], na.rm=TRUE),
                   avg_ndi_net4=mean(ndi[grepl("_4", roi)], na.rm=TRUE), 
                   avg_ndi_net5=mean(ndi[grepl("_5", roi)], na.rm=TRUE), 
                   avg_ndi_net6=mean(ndi[grepl("_6", roi)], na.rm=TRUE),
                   avg_ndi_net7=mean(ndi[grepl("_7", roi)], na.rm=TRUE), 
                   avg_ndi_net8=mean(ndi[grepl("_8", roi)], na.rm=TRUE), 
                   avg_ndi_net9=mean(ndi[grepl("_9", roi)], na.rm=TRUE),
                   avg_ndi_net10=mean(ndi[grepl("_10", roi)], na.rm=TRUE), 
                   avg_ndi_net11=mean(ndi[grepl("_11", roi)], na.rm=TRUE), 
                   avg_ndi_net12=mean(ndi[grepl("_12", roi)], na.rm=TRUE),
                   avg_ndi_net13=mean(ndi[grepl("_13", roi)], na.rm=TRUE), 
                   avg_ndi_net14=mean(ndi[grepl("_14", roi)], na.rm=TRUE))
