...
## NOTE: this script is step 1 in constructing correlation matrices from just the five core networks of interest (frontoparietal, cingulo-opercular, default mode, somatomotor dorsal, visual, 214 ROIs total)
## NOTE TO CLEANTHIS: make sure to change the file names when saving to differentiate from the 300ROI analyses + make sure to update the timepoints per scan according to your conversation with Jess (this script is set at 50 tps per scan + 78 total)
...
for(i in 1:length(sub_list)){
  ...
  for(j in 1:length(ses_list)){
    ...
    for(l in 1:length(task_list)){
      ...
      goodtps = 0 # goodtps is specified per task below
      goodtps_scan = 0 # goodtps_scan is specified per run below
      ...
      if(length(file_inds) >= 1){ ## asks how many runs in this subject/session/task
        ...
        for(k in 1:length(file_inds)){ ## looping through each run of the task in the loop above
          data_allnets <- read.csv(file_info$roifiles[file_inds[k]], stringsAsFactors = F, header = F)
          ##THIS PART IS NEW FOR THIS ANALYSIS: selecting 214 ROIs from five networks and removing all other ROIs
          data = data_allnets %>% select(13:44, 50:63, 76:130, 136:196, 238, 239, 248:273, 277:300)
          ...
          if(goodtps_scan >= 30){ # dont want to remove runs that have less than 78; want to take all tps from each run and add them together; then cutoff the total tps from the concatenated task runs t >=78 ---- after discussion with JRC, keep 30tps per run and 78tps in total
...
sub_datalist_30tpsperrun = sub_datalist
ROI_TS_list_30tpsperrun = ROI_TS_list
save(sub_datalist_30tpsperrun, ROI_TS_list_30tpsperrun, file = "ConcatenatedROItimeseries_30tpsperrun.RData")
...
load(file = "ConcatenatedROItimeseries_30tpsperrun.RData")
sub_datalist = sub_datalist_30tpsperrun
    ...
    for(l in 1:length(sub_datalist[[i]][[j]])){
      if(length(sub_datalist[[i]][[j]]) >= 1){
        ...
        tryCatch({
          ...
          file_name = paste("./correlation_matrices30tps/", names(sub_datalist)[i], "_", names(sub_datalist[[i]])[j], "_", names(sub_datalist[[i]][[j]])[l], ".csv", sep = "")
